...
function cmdSetup({ claude = false } = {}) {
...
function cmdInitSymlink(targetArg, { claude = false } = {}) {
...
function cmdInitCopy(targetArg, { claude = false } = {}) {
  const target = resolve(targetArg);
  if (!existsSync(target)) {
    console.error(`Error: target path does not exist: ${target}`);
    process.exit(1);
...
Commands:
  ...
  inspectra setup --claude           Generate CLAUDE.md + .mcp.json in the current directory for Claude Code
  inspectra init <project>           Symlink agents into a project (gitignored) + .vscode/mcp.json
  inspectra init <project> --copy    Copy agents into a project (committed with the repo)
  inspectra init <project> --claude  Set up project for Claude Code (.mcp.json + CLAUDE.md instead of .vscode/mcp.json)
...
if (!cmd || cmd === "--help" || cmd === "-h") {
  ...
  cmdSetup({ claude: hasFlag("--claude") });
} else if (cmd === "doctor") {
  cmdDoctor();
} else if (cmd === "init") {
  const target = positional[1];
  if (!target) { console.error("Usage: inspectra init <project-path> [--copy] [--claude]"); process.exit(1); }
  const claudeFlag = hasFlag("--claude");
  if (hasFlag("--copy")) {
    cmdInitCopy(target, { claude: claudeFlag });
  } else {
    cmdInitSymlink(target, { claude: claudeFlag });
  }
} else {
